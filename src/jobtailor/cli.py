...
@app.command()
def analytics(
    period: str = typer.Option("all", help="Period: 'all', 'today', or '7d'"),
):
    """Show response rate analytics and conversion funnel."""
    from jobtailor.analytics import AnalyticsEngine

    engine = AnalyticsEngine()
    if period == "today":
        from datetime import date
        data = engine.daily_summary(date.today().isoformat())
    elif period == "7d":
        data = engine.source_effectiveness()
    else:
...
@app.command()
def export(
    format: str = typer.Option("both", help="Format: 'json', 'csv', or 'both'"),
    output: str = typer.Option("output/export", help="Output path (without extension)"),
):
    """Export application history to JSON and/or CSV."""
    from jobtailor.export import export_csv, export_json

    results = []
    if format in ("json", "both"):
        path = export_json(output_path=f"{output}.json")
        results.append(f"JSON: {path}")
    if format in ("csv", "both"):
        path = export_csv(output_path=f"{output}.csv")
...
@app.command()
def versions(
    action: str = typer.Argument("list", help="Action: 'list', 'add', 'remove', 'analytics', 'best'"),
    name: str = typer.Option("", help="Version name (for add/remove)"),
    path: str = typer.Option("", help="Resume path (for add)"),
    description: str = typer.Option("", help="Version description (for add)"),
    tags: str = typer.Option("", help="Comma-separated tags (for add)"),
):
    """Manage resume versions and A-B testing."""
    ...
    if action == "list":
        ...
        if not versions:
            typer.echo("No resume versions configured.")
            return
        for v in versions:
            typer.echo(f"  {v.name}: {v.path} ({', '.join(v.tags) or 'no tags'})")

    elif action == "add":
        if not name or not path:
            typer.echo("Name and path are required for 'add'.")
            raise typer.Exit(1)
        tag_list = [t.strip() for t in tags.split(",") if t.strip()]
        mgr.add_version(name, path, description, tag_list)
        typer.echo(f"Added version: {name}")

    elif action == "remove":
        if not name:
            typer.echo("Name is required for 'remove'.")
            raise typer.Exit(1)
        if mgr.remove_version(name):
            typer.echo(f"Removed version: {name}")
        else:
            typer.echo(f"Version not found: {name}")

    elif action == "analytics":
        import json
        typer.echo(json.dumps(mgr.analytics(), indent=2))

    elif action == "best":
        best = mgr.best_version()
        if best:
            typer.echo(f"Best version: {best}")
        else:
            typer.echo("No version has enough data (need 3+ applications).")


@app.command()
def cover_letters(
    config: str = typer.Option("config.yaml", help="Path to config.yaml"),
    jds_dir: str = typer.Option("output/jds", help="Directory with extracted JDs"),
    profile: str = typer.Option("", help="Candidate profile (2-3 sentences)"),
):
    """Generate cover letters for extracted job descriptions."""
    from jobtailor.coverletter import CoverLetterGenerator
    from jobtailor.config import load_config

    cfg = load_config(config)
    if not profile:
        profile = cfg.applicant.fit_profile
    ...
    typer.echo(f"Generating cover letters for {len(jds)} jobs...")
    gen = CoverLetterGenerator(llm_config=LLMConfig(provider=cfg.llm.provider, model=cfg.llm.model))
    results = gen.generate_batch(jds, profile)

    ok = sum(1 for r in results if r["ok"])
    typer.echo(f"Generated {ok}/{len(results)} cover letters:")
    for r in results:
        status = "ok" if r["ok"] else "FAILED"
        typer.echo(f"  [{status}] {r['company']} | {r['title']}")
        if r.get("path"):
            typer.echo(f"           -> {r['path']}")
...
def score(
    config: str = typer.Option("config.yaml", help="Path to config.yaml"),
    leads_file: str = typer.Option("", help="Path to leads JSON (default: latest)"),
):
    """Score and rank leads by fit (salary, tech match, remote, etc.)."""
    from jobtailor.scoring import ScoringWeights, ScoringProfile, rank_jobs
    ...
    weights = ScoringWeights(
        salary=cfg.scoring.salary_weight,
        tech_match=cfg.scoring.tech_match_weight,
        remote=cfg.scoring.remote_weight,
        company_size=cfg.scoring.company_size_weight,
        title_match=cfg.scoring.title_match_weight,
        source_reliability=cfg.scoring.source_reliability_weight,
    )
    profile = ScoringProfile(
        preferred_salary_min=cfg.scoring.preferred_salary_min,
        preferred_salary_max=cfg.scoring.preferred_salary_max,
        preferred_tech=cfg.scoring.preferred_tech,
        preferred_titles=cfg.scoring.preferred_titles,
    )

    jobs = [{"title": l.title, "company": l.company, "source": l.source, "remote": l.remote} for l in leads]
    ranked = rank_jobs(jobs, weights, profile)

    typer.echo(f"Scored {len(ranked)} leads:")
    for j in ranked[:20]:
        typer.echo(f"  [{j['score']:.3f}] {j['title']} @ {j['company']} ({j['source']})")


@app.command()
def review(
    tailored_dir: str = typer.Option("output/tailored", help="Tailored resume directory"),
    resume_dir: str = typer.Option("", help="Original resume directory (default: from config)"),
    config: str = typer.Option("config.yaml", help="Path to config.yaml"),
):
    """Review tailoring diffs before PDF export."""
    from jobtailor.review import review_tailored_resume, save_review

    cfg = load_config(config)
    original = resume_dir or cfg.resume.source

    tailored_path = Path(tailored_dir)
    if not tailored_path.exists():
        typer.echo(f"No tailored resumes found at {tailored_dir}/")
        raise typer.Exit(1)

    branches = sorted(p.relative_to(tailored_path) for p in tailored_path.glob("apply/*/resume.tex"))
    if not branches:
        typer.echo("No tailored resumes to review.")
        return

    typer.echo(f"Reviewing {len(branches)} tailored resumes:")
    for branch in branches:
        branch_str = str(branch).replace("/resume.tex", "")
        review_obj = review_tailored_resume(
            tailored_path / branch_str,
            original,
            branch=branch_str,
        )
        path = save_review(review_obj)
        typer.echo(f"  {branch_str}: {review_obj.total_changes} changes -> {path}")
